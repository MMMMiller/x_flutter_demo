...
- (void)__setAssociatedState:(ControlState)state {
    objc_setAssociatedObject(self, "x_state", @(state), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (ControlState)__associatedState {
    NSNumber * state = objc_getAssociatedObject(self, "x_state");
...
- (UIButton * _Nonnull (^)(UIFont * _Nonnull))x_font {
