...
- (void)preloadFlutterModuleAfterDelay:(CGFloat)delay {
    dispatch_block_t preload = ^(){
        [FlutterModuleNavigator push:@"main/root_page" arguments:@{}];
        ...
        [self jk_excuteDelayTaskWithKey:"dispose_flutter_module" delayInSeconds:0.6 inMainQueue:^{
            ...
            // [FlutterBoost.instance.plugin.delegate popRoute:@"main/root_page"];
        }];
    };
...
- (void)pushNativeRoute:(NSString *)pageName arguments:(NSDictionary *)arguments {
    NSLog(@"打开原生页面");
    id page = NSClassFromString(pageName);
//    id page = [[NativeViewController alloc] init];
    if ([[page class] isSubclassOfClass:UIViewController.class]) {
        [FlutterModuleHelper.visibleViewController.navigationController pushViewController:[[page alloc] init]  animated:YES];
        
        //参数接收和处理
        //    NSMutableDictionary * parameters = SafeMutDictionary(arguments);
    } else {
        NSLog(@"⚠️ FlutterBoost pushNativeRoute：arguments：路由未被处理");
    }
}
...
- (void)popRoute:(FlutterBoostRouteOptions *)options {
    if (options.uniqueId == nil || options.uniqueId.length == 0) {
        NSLog(@"⚠️ FlutterBoost popRoute：退出失败，uniqueId为空");
        return;
    }
    
    FlutterModuleViewController * flutterVC = [self.vcHashMap objectForKey:options.uniqueId];
    ...
    /// 对root_page 特殊处理
    if ([flutterVC.route isEqualToString:@"main/root_page"]) {
        [flutterVC.view removeFromSuperview];
        [flutterVC removeFromParentViewController];
        [self.vcHashMap removeObjectForKey:options.uniqueId];
    }
...
- (void)pushFlutterRoute:(FlutterBoostRouteOptions *)options { 
    ...
    if (present) { // 处理present
        [self presentFlutterRoute:options.pageName arguments:options.arguments completion:options.completion];
        return;
    }
    
    
    if ([options.pageName isEqualToString:@"main/root_page"]) {
        CGFloat screenWidth = CGRectGetWidth(UIScreen.mainScreen.bounds);
        CGFloat screenHeight = CGRectGetHeight(UIScreen.mainScreen.bounds);
        
        /// 尽量在启动APP后就进行FlutterModule预加载，以及切换深浅色主题后，提前渲染一个Flutter页面
        /// 以解决进入Flutter页面背景色延迟改变的问题
        FlutterModuleViewController * flutterVC = [[FlutterModuleViewController alloc] initWithRoute:options.pageName params:options.arguments];
        // 放在屏幕边缘
        ...
        [self.vcHashMap setObject:flutterVC forKey:flutterVC.flutterContainer.uniqueIDString];
        SafeBlockPerformer(options.completion, true);
        return;
    }
    
    
    // 转场动效
    BOOL animated = options.arguments[@"animated"] == nil ? true : [options.arguments[@"animated"] boolValue];
